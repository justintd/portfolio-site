  ...
  return (
    <>
      <nav className="lg:hidden">
        ...
        <aside
          className={
            `transform top-0 bottom-0 right-0 w-8/12 md:w-5/12 bg-black-2 fixed h-screen overflow-auto ease-in-out transition-all duration-300 z-30 ` +
          ...
          <ul className="flex flex-col h-full items-center justify-center align-center">
            {Object.keys(props.links).map((key, value) => {
              if (!props.links[key].button) {
                ...
                return (
                  <li className="my-3">
                    <button className="px-5 py-2 text-lg rounded-sm border focus:outline-none text-melon tracking-wide hover:bg-melon hover:bg-opacity-25 transition-all ease-in-out duration-300">
        ...
        <Fade top>
          ...
          >
            <span
              className={
                `block bg-white w-6 h-1 my-1 mx-auto transition-all ease-in-out duration-300 ` +
                (props.isActive ? "transform translate-y-2 rotate-45" : null)
              }
            ></span>
            <span
              className={
                `block bg-white w-6 h-1 my-1 mx-auto ease-in-out duration-300 ` +
